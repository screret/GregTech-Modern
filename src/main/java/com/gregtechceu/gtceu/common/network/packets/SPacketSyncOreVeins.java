...
public class SPacketSyncOreVeins implements GTNetwork.INetPacket {
    ...
    public SPacketSyncOreVeins(FriendlyByteBuf buf) {
        ...
        Stream.generate(() -> {
            ...
            GTOreDefinition def = GTOreDefinition.DIRECT_CODEC.parse(ops, tag).getOrThrow(false, GTCEu.LOGGER::error);
    ...
    public void encode(FriendlyByteBuf buf) {
        ...
        for (var entry : veins.entrySet()) {
            ...
            CompoundTag tag = (CompoundTag) GTOreDefinition.DIRECT_CODEC.encodeStart(ops, entry.getValue())
